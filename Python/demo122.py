...
def main(*args):
    try:
        ...
        with open(path+"text19.txt", "w") as file:
            for date in lst_date:
                json.dump(date, file, default=encode_date)
                file.write('\n')
        
        with open(path+"text19.txt", "r+") as file:
            while True:
                line = file.readline()
                if line == "":
                    break
                file_content = json.loads(line, object_hook=decode_date)
                print(file_content)
